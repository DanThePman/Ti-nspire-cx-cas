    ...
    for _, correspondingIndex in ipairs(directionOutputCorrespondingIndex) do
        ...
        local dirDirVecIndex = correspondingIndexCount * 2 - 1
            ...
            if opVecX + dirVecX == xComponent and opVecY + dirVecY == yComponent and dirVecX > 0 and dirVecY > 0 then --op + dir = parameter vec
                    local completeOpVec = { opVecX, opVecY }
                    local completeDirVec = { dirVecX, dirVecY }
                return completeOpVec, completeDirVec
            end
...
local opVecBaseVec, dirVecBaseVec = IsOp_Dir_Vec(baseVec[X], baseVec[Y])
local opInteractVec, dirInteractVec = IsOp_Dir_Vec(interactingVec[X], interactingVec[Y])

if opVecBaseVec and dirVecBaseVec and opInter and dirInteractVec then --both vectors are resulting vecs of op + dir (they are ops btw)
    
    if not VecsAreCollinear(dirVecBaseVec[X], dirVecBaseVec[Y],   dirInteractVec[X], dirInteractVec[Y]) then --dir vecs will ever face each other
    
    local k = (dirInteractVec[X] * (opInteractVec[Y] - opVecBaseVec[Y]) - dirInteractVec[Y] * (opInteractVec[X] - opVecBaseVec[X]) )/
        (dirInteractVec[X]* dirVecBaseVec[Y] - dirInteractVec[Y] * dirVecBaseVec[X])
        
    local l = (dirVecBaseVec[X] * (opInteractVec[Y] - opVecBaseVec[Y]) - dirVecBaseVec[Y] * (opInteractVec[X] - opVecBaseVec[X])) /
        (dirInteractVec[X] * dirVecBaseVec[Y] - dirInteractVec[Y] * dirVecBaseVec[X])
        
        gc:drawString("Veks treffen sich bei:\n("..opVecBaseVec[X].."|"..opVecBaseVec[Y]..") + "..k.." * ("..
                                                                    dirVecBaseVec[X].."|"..dirVecBaseVec[Y]..")\n("..
                                                                    
                                                                    opVecBaseVec[X].."|"..opVecBaseVec[Y]..") + "..l.." * "..
                                                                    dirInteractVec[X].."|"..dirInteractVec[Y]..")", 5 ,40+35*2,"top")
        
    else --dir vecs will never face each other
        gc:drawString("Dir Veks sind kollinear", 5 ,40+35*2,"top")
    end
